...
> - 🔲 **Not yet implemented:** Service mode, service install/uninstall, Tray ↔ Agent IPC, gRPC transport, streaming progress events, undo, dry-run, restore points, execution audit log, Authenticode client verification, code signing, Sysinternals tool resolver, remaining operations (`registry-read`, `service-*`, `file-*`, `sysinternals-tool`, `dism`, `system-restore-point`), sub-action step execution, template functions (`drive()` / `basename()` / `dirname()`), type-specific parameter pickers in the UI.
>
